...
Deno.serve(async (req) => {
  ...
  const { data: room } = await supabaseAdmin
    .from('rooms')
    .select('id, code, host_id, status, current_round, used_question_ids, pack')
    .eq('id', room_id)
    .single()

  if (!room) return json({ error: 'ROOM_NOT_FOUND' }, 404)
  if (room.host_id !== user.id) return json({ error: 'FORBIDDEN' }, 403)
  if (room.status === 'closed' || room.status === 'lobby') return json({ error: 'ROOM_NOT_ACTIVE' }, 409)
  ...
  if (!roomPlayers || roomPlayers.length === 0) return json({ error: 'NO_PLAYERS' }, 400)
  ...
  await new Promise<void>((resolve) => {
    channel.subscribe(async (status) => {
      if (status === 'SUBSCRIBED') {
        await channel.send({ type: 'broadcast', event: 'round:started', payload: broadcastPayload })
